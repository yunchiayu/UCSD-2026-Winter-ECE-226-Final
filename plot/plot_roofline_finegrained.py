...
def parse_args():
    ...
    parser.add_argument("--output-dir", type=str, default="./figures/roofline_finegrained")
...
def plot_roofline(args, hardware_configs, point_datas, output_path: str, plot_config: dict, legend_config: dict):
    ...
    if model_handles:
        model_leg = fig.legend(
            ...
            loc='lower right',
            bbox_to_anchor=(0.92, 0.00),
            ncol=min(2, len(model_handles)),
            frameon=False,
            title='Kernel Types (Prompt Length: 1024, 2048, 4096, 8192)',
            fontsize=fontsize.get("legend_fontsize", 16),
            title_fontsize=fontsize.get("legend_title_fontsize", 18),
        )
...
def main(args):
    ...
    kernel_colors = {
        "gemm": ["#BFDBFE", "#60A5FA", "#2563EB", "#1E3A8A"], # blue
        "qK_softmax_sV_fused": ["#E8F5E9", "#A5D6A7", "#66BB6A", "#2E7D32"], # green
        "mamba_elementwise": ["#FEF3E2", "#F6C28B", "#E6953B", "#A85A17"], # orange
        "conv1d": ["#F5F5F5", "#CFCFCF", "#7A7A7A", "#2F2F2F"]
    }
    ...
    for model_idx, model_name in enumerate(args.model_name_list): 
        for sum_seq_len_idx, sum_seq_len in enumerate(args.sum_seq_len_list):
            ...
            for kernel in roofline_data["kernels"]:
                ...
                # ===== Color Setting =====
                kernel_type = kernel.get("type", "other")
                if kernel_type not in kernel_colors:
                    kernel_type = "other"
                palette = kernel_colors[kernel_type]
                color = palette[sum_seq_len_idx % len(palette)]
    ...
    model_labels = []
    # for model_idx, model_name in enumerate(args.model_name_list):
    for kernel_type in kernel_colors:
        ...
        if kernel_type == "gemm":
            model_label = "GEMM"
        elif kernel_type == "qK_softmax_sV_fused":
            model_label = "Fused Attention"
        elif kernel_type == "mamba_elementwise":
            model_label = "Elementwise Operations"
    ...
    if len(args.model_name_list) > 1:
        filename = f"roofline_all.png"
    else:
        filename = f"roofline_{model_name_table[args.model_name_list[0]]}.png"
    output_path = Path(args.output_dir) / filename
    output_path.parent.mkdir(parents=True, exist_ok=True)

    plot_roofline(args, hardware_configs, point_datas, output_path, plot_config, legend_config)
